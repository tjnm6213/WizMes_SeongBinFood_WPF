...
namespace WizMes_SeongBinFood
{
    ...
    public partial class Win_ord_Order_U : UserControl
    {
        ...
        private void btnPreOrderOK_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (dtpThisMonth.SelectedDate.Value == dtpThisMonth.SelectedDate.Value)
                {
                    MessageBox.Show("같은달에 수주를 복사할 수 없습니다.");
                    return;
                }
        ...
        private void CanBtnControl()
        {
            ...
            //btnStillAdd.IsEnabled = true;
            //btnStillAdd.IsHitTestVisible = true;
        ...
        private void CantBtnControl()
        {
            ...
            //btnStillAdd.IsEnabled = false;
            //btnStillAdd.IsHitTestVisible = false;
        ...
        // 유지추가
        private void btnStillAdd_Click(object sender, RoutedEventArgs e)
        {
            strFlag="I";
        ...
        private void btnSave_Click(object sender, RoutedEventArgs e)
        {
            if (SaveData(strFlag))
            {
                if (CheckBoxRemainData.IsChecked == false)
                {
                    CanBtnControl();
                    lblMsg.Visibility = Visibility.Hidden;
                    dgdMain.IsHitTestVisible = true;
                ...
                {
                    MessageBox.Show("저장이 완료되었습니다. 입력 데이터를 보존합니다.");
                }
        ...
        private void btnCancel_Click(object sender, RoutedEventArgs e)
        {
            ...
            dgdMain.IsHitTestVisible = true;
            btnNeedStuff.IsEnabled = false;
            CheckBoxRemainData.IsChecked = false;
        ...
        private bool SaveData(string strFlag)
        {
            ...
            try
            {
                if (CheckData())
                {
                    ...
                    sqlParameter.Add("OrderID", txtOrderID.Text == string.Empty ? "" : txtOrderID.Text); // 관리번호
                    sqlParameter.Add("OrderNO", TextBoxOrderNo.Text == string.Empty ? "" : TextBoxOrderNo.Text); // OrderNo
                    sqlParameter.Add("CustomID", txtCustom.Tag.ToString()); //납품거래처
                    sqlParameter.Add("DvlyPlace", txtDylvLoc.Text); // 납품장소
                    sqlParameter.Add("InCustomID", txtInCustom.Tag.ToString()); // 매출거래처

                    sqlParameter.Add("AcptDate", dtpAcptDate.SelectedDate.Value.ToString("yyyyMMdd")); // 접수 일자
                    sqlParameter.Add("DvlyDate", chkDvlyDate.IsChecked == true ? dtpDvlyDate.SelectedDate.Value.ToString("yyyyMMdd") : ""); // 납기 일자
                    sqlParameter.Add("WorkID", cboWork.SelectedValue.ToString()); // 가공 구분
                    sqlParameter.Add("BuyerID", txtBuyerCustom.Text == string.Empty ? "" : txtBuyerCustom.Tag.ToString()); // 최종 거래처
                    
                    sqlParameter.Add("OrderForm", cboOrderForm.SelectedValue.ToString()); // 주문 형태
                    sqlParameter.Add("OrderClss", cboOrderClss.SelectedValue.ToString()); // 주문 구분
                    sqlParameter.Add("OrderQty", int.Parse(txtAmount.Text.Replace(",", ""))); // 총 주문량
                    sqlParameter.Add("UnitClss", cboUnitClss.SelectedValue.ToString()); // 주문 기준
                    sqlParameter.Add("ArticleGrpID", cboArticleGroup.SelectedValue.ToString()); // 품명 종류

                    sqlParameter.Add("UnitPrice", ConvertDouble(txtUnitPrice.Text)); // 단가
                    sqlParameter.Add("PoNo", txtPONO.Text); // P/O NO.
                    ...
                    if (strFlag.Equals("I"))
                    {
                        ...
                        for (int i = 0; i < DataGridSub.Items.Count; i++)
                        {
                            ...
                            sqlParameter.Add("ModelID", OrderSub.ModelID);
                            ...
                            sqlParameter.Add("ColorQty", ConvertInt(OrderSub.ColorQty));
                ...
                if (strFlag == "I" || strFlag == "U")
                {
                    ...
                    if (e.Key == Key.Enter)
                    {
                        ...
                        if (dt.Rows.Count != 0)
                        {
                            ...
                            SubItem.ArticleID = dt.Rows[0]["ArticleID"].ToString();
        ...
        private void DataGridSubTextBoxModel_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (strFlag == "I" || strFlag == "U")
                {
                    ...
                    {
                        TextBox tb1 = sender as TextBox;
                        pf.ReturnCode(tb1, 28, SubItem.Model);

                        if (tb1.Tag != null)
                        {
                            SubItem.ModelID = tb1.Tag.ToString();
                            SubItem.Model = tb1.Text;
                        }
        ...
        private void txtBuyCustom_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                MainWindow.pf.ReturnCode(txtBuyerCustom, 72, "");
            }
        }
        ...
        //유지추가 라벨 클릭
        private void LabelRemainData_MouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            try
            {
                if (CheckBoxRemainData.IsChecked == true)
                {
                    CheckBoxRemainData.IsChecked = false;
                }
                else
                {
                    CheckBoxRemainData.IsChecked = true;
                }
            }
            catch (Exception ee)
            {
                MessageBox.Show("오류지점 - LabelRemainData_MouseLeftButtonDown : " + ee.ToString());
            }
        }
        //유지추가 체크
        private void CheckBoxRemainData_Checked(object sender, RoutedEventArgs e)
        {
            try
            {
                CheckBoxRemainData.IsChecked = true;
            ...
            {
                MessageBox.Show("오류지점 - CheckBoxRemainData_Checked : " + ee.ToString());
            }
        }
        //유지추가 체크해제
        private void CheckBoxRemainData_Unchecked(object sender, RoutedEventArgs e)
        {
            try
            {
                CheckBoxRemainData.IsChecked = false;
            ...
            {
                MessageBox.Show("오류지점 - CheckBoxRemainData_Unchecked : " + ee.ToString());
            }
        }
    }
