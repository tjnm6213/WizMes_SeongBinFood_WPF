...
namespace WizMes_SeongBinFood
{
    ...
    public partial class Win_ord_OrderClose_U : UserControl
    {
        ...
        WizMes_SeongBinFood.PopUp.NoticeMessage msg = new PopUp.NoticeMessage();
                    ...
                    foreach (Win_ord_OrderClose_U_CodeView OrderCloseU in dgdMain.Items)
                    {
                        if (OrderCloseU != null)
                        {
                            if (OrderCloseU.IsCheck == true)
                            {
                                ...
                                sqlParameter.Add("OrderSeq", OrderCloseU.OrderIDSeq);
        ...
        private void FillGrid()
        {
            ...
            try
            {
                ...
                sqlParameter.Add("ChkInCustom", chkInCustom.IsChecked == true ? (txtInCustom.Tag != null ? 1 : 2) : 0);
                sqlParameter.Add("InCustomID", chkInCustom.IsChecked == true ? (txtInCustom.Tag != null ? txtInCustom.Tag.ToString() : txtInCustom.Text) : "");
                ...
                if (ds != null && ds.Tables.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        foreach (DataRow item in drc)
                        {
                            ...
                            Window_OrderClose_DTO.OrderQty = stringFormatN0(Lib.Instance.returnNumStringZero(Window_OrderClose_DTO.OrderQty));
                            Window_OrderClose_DTO.ColorQty = stringFormatN0(Lib.Instance.returnNumStringZero(Window_OrderClose_DTO.ColorQty));
            ...
            try
            {
                ...
                for (int k = 0; k < PageCount; k++)
                {
                    ...
                    for (int i = DataCount; i < dgdMain.Items.Count; i++)
                    {
                        ...
                        str_KCustom = DT.Rows[i][4].ToString();
                        str_ArticleNo = DT.Rows[i][7].ToString();
                        str_Article = DT.Rows[i][8].ToString();
                        str_Model = DT.Rows[i][9].ToString();


                        str_DvlyDate = DT.Rows[i][10].ToString();
                        str_Work = DT.Rows[i][11].ToString();
                        str_OrderQty = DT.Rows[i][12].ToString();
                        str_UnitClssName = DT.Rows[i][13].ToString(); //안씀
                        str_DayAndTime = DT.Rows[i][14].ToString();
                        str_p1WorkQty = DT.Rows[i][15].ToString();
                        str_InspectQty = DT.Rows[i][16].ToString();
                        str_PassQty = DT.Rows[i][17].ToString();
                        str_DefectQty = DT.Rows[i][18].ToString();
                        str_OutQty = DT.Rows[i][19].ToString();
    ...
    class Win_ord_OrderClose_U_CodeView : BaseView
    {
        ...
        public string OrderIDSeq { get; set; }
