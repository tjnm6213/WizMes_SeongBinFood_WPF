    ...
    class ComboBoxUtil
    {
                                              ...
                                              string psUseYN, string psParentID, string psRelation = "")
        {
            ...
            */
            ...
            sql += " AND Code_ID <> '0000'"; //2022.05.09 정승학 추가
        ...
        /// </summary>
        /// <returns></returns>
        public ObservableCollection<CodeView> GetInfoDefect()
        {
            ObservableCollection<CodeView> returnCollection = new ObservableCollection<CodeView>();

            string strSqlMSG = "";
            strSqlMSG += " SELECT DefectID As Code_ID,KDefect AS CODE_NAME,Display1 ";
            strSqlMSG += " FROM mt_defect ";
            strSqlMSG += " WHERE 1 = 1 ";
            strSqlMSG += " AND DefectID <>'000' ";

            DataSet ds = DataStore.Zip_Instance.QueryToDataSet(strSqlMSG);
            if (ds != null && ds.Tables.Count > 0)
            {
                DataTable dt = ds.Tables[0];
                if (dt.Rows.Count == 0)
