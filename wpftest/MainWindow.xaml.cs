    ...
    public partial class MainWindow : Window
    {
        ...
        private void Win_Com_info()
        {
            ...
            try
            {
                ...
                {
                    Type type = Type.GetType("WizMes_SeongBinFood." + MainWindow.mMenulist[i].ProgramID.Trim(), true);
                    object uie = Activator.CreateInstance(type);

                    MainWindow.mMenulist[i].subProgramID = new MdiChild()
                    {
                        Title = "WizMes_SeongBinFood [" + MainWindow.mMenulist[i].MenuID.Trim() + "] " + MainWindow.mMenulist[i].Menu.Trim() +
        ...
        private void fmenu_click(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            else
            {
                try
                {
                    tt1 = Type.GetType("WizMes_SeongBinFood." + MenuViewModel.ProgramID.Trim(), true);
                    uie = Activator.CreateInstance(tt1);
              
                    MenuViewModel.subProgramID = new MdiChild()
                    { 
                        Title = "WizMes_SeongBinFood [" + MenuViewModel.MenuID + "] " + MenuViewModel.Menu + " (→" + MenuViewModel.ProgramID.Trim() + ")",
                        Height = SystemParameters.PrimaryScreenHeight * 0.8,
                        MaxHeight = SystemParameters.PrimaryScreenHeight * 0.85,
                        MinHeight = 640,
                        Width = SystemParameters.WorkArea.Width * 0.85,
        ...
        private void CommonClick(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            strPath = strPath + "\\WizMes_SeongBinFood2.exe";
                    ...
                    else
                    {
                        ...
                        foreach (DataRow item in drc)
                        {
                            var mMenuviewModel = new MenuViewModel()
                            {
                                ...
                                ////Remark = "WizMes_SeongBinFood." + item["Remark"].ToString(),
        ...
        protected override void OnClosing(CancelEventArgs e1)
        {
            try
            {
                if (MessageBox.Show("WizMes_SeongBinFood를 종료하시겠습니까?", "", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
                {
                    Environment.Exit(0);
                    System.Diagnostics.Process.GetCurrentProcess().Kill();
                }
