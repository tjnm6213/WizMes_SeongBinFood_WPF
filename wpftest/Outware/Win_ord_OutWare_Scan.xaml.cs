...
namespace WizMes_SeongBinFood
{
    ...
    public partial class Win_ord_OutWare_Scan : UserControl
    {
        ...
        WizMes_SeongBinFood.PopUp.NoticeMessage msg = new WizMes_SeongBinFood.PopUp.NoticeMessage();
        ...
        private void dgdOutware_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                ...
                if (OutwareInfo != null)
                {
                    ...
                    //txtBuyerModel.Tag = OutwareInfo.ModelID;
                    txtBuyerModel.Text = OutwareInfo.Model;
            ...
            try
            {
                if (e.Key == Key.Enter)
                {
                    ...
                    if (tgnMoveByQty.IsChecked == true)
                    {
                        ...
                        if (CheckConvertInt(txtScanData.Text))
                        {
                            // 수량 입력시 라벨 없이 입력됨
        ...
        private void CalculGridSum()
        {
            ...
            txtOrderCount.Text = string.Format("{0:N0}", dgdOutware.Items.Count) + " 건";
            if (dgdOutware.Items.Count > 0)
            {
                Win_ord_OutWare_Scan_CodeView WinOrder = new Win_ord_OutWare_Scan_CodeView();

                for (int i = 0; i < dgdOutware.Items.Count; i++)
                {
                    WinOrder = dgdOutware.Items[i] as Win_ord_OutWare_Scan_CodeView;

                    if (WinOrder.UnitClss.Equals("0"))
                    {
                        numYDS += Int64.Parse(WinOrder.OutQty.Replace(",", ""));
                    ...
                    if (strFlag == "I")
                    {
                        ...
                        if (tgnMoveByID.IsChecked == true || tgnMoveByQty.IsChecked == true)
                        {
                            ...
                            for (int i = 0; i < dgdOutwareSub.Items.Count; i++)
                            {
                                ...
                                sqlParameter.Add("LabelID", OutwareSub.LabelID == null ? "" : OutwareSub.LabelID);
        ...
        private void tgnMovePartial_Click(object sender, RoutedEventArgs e)
        {
            ...
            // 그리드 변경
            dgdOutwareSub.Visibility = Visibility.Hidden;
